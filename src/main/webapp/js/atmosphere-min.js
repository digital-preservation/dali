(function(){var c="2.0.5-javascript",a={},d,g=[],f=[],e=0,b=Object.prototype.hasOwnProperty;
...
if(O.reconnectInterval>0){O.id=setTimeout(function(){I(true)
},O.reconnectInterval)
}else{I(true)
}W.responseBody="";
W.messages=[]
...
}Z.onopen=function(aD){y(O);
if(O.logLevel==="debug"){a.util.debug("Websocket successfully opened")
}if(!O.enableProtocol){if(!aB){N("opening","websocket",O)
}else{N("re-opening","websocket",O)
}}aB=true;
...
if(O.reconnectInterval>0){O.id=setTimeout(function(){W.responseBody="";
W.messages=[];
aj(true)
},O.reconnectInterval)
}else{W.responseBody="";
...
W.messages=[];
C()
}function x(aE,aD,aA){if(!u(O,aE)){return true
...
if(aB>0){O.id=setTimeout(function(){t()
},aB)
}else{t()
...
if(aA.transport!=="polling"){if((!aA.enableProtocol||!aC.firstMessage)&&aB.readyState===2){ao(aA)
}var aF=200;
if(aB.readyState>1){aF=aB.status>1000?0:aB.status
...
clearTimeout(aC.id);
if(aD>0){aC.id=setTimeout(function(){s(aC)
},aD)
}else{s(aC)
...
if(l++<aB.maxReconnectOnClose){if(aB.reconnectInterval>0){aB.id=setTimeout(function(){N("re-connecting",aC.transport,aC);
P(aB)
...
if(aC.reconnectInterval>0){aC.id=setTimeout(function(){av(aC)
...
N("re-connecting",aC.transport,aC);
if(l++<aC.maxReconnectOnClose){if(aC.reconnectInterval>0){aC.id=setTimeout(function(){av(aC)
},aC.reconnectInterval)
}else{av(aC)
}}else{af(0,"maxReconnectOnClose reached")
}aE.execCommand("Stop");
...
case"opening":if(typeof(aC.onOpen)!=="undefined"){aC.onOpen(aB)
}break;
...
case"re-opening":if(typeof(aC.onReopen)!=="undefined"){aC.onReopen(O,aB)
}break;
...
},isBinary:function(i){var h=Object.prototype.toString.call(i);
return h==="[object Blob]"||h==="[object ArrayBuffer]"
